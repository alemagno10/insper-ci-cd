...
public class GameControllerTests {
    ...
    //     Mockito.when(gameService.listGames(Mockito.any(String.class), Mockito.any(String.class), Mockito.any(Integer.class), Mockito.eq(paginacao)))
    ...
    //             .param("home", "Kansas")
    //             .param("away", "Dallas")
    //             .param("page", "0") // Número da página desejada
    //             .param("size", "10"))
    //         .andExpect(MockMvcResultMatchers.status().isOk())
    //         .andReturn();
    ...
    //     String resp = result.getResponse().getContentAsString();
    //     Assertions.assertEquals(om.writeValueAsString(games), resp);
    // }
